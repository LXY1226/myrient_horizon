...
type FileNode struct {
	...
	// Mutable aggregation state (managed by server tree).
	BestStatus  uint8
	ReportCount uint8
	HasConflict bool
}
...
type Tree struct {
	...
	Dirs    []DirNode  // dirs[dirID]
	Files   []FileNode // files[fileID]
...
func LoadFromBytes(data []byte) (*Tree, error) {
	...
	t.Dirs = make([]DirNode, 0, 65536)
	t.Files = make([]FileNode, 0, 3000000)
...
func (t *Tree) buildDFS(fbNode *fb.Node, parentIdx int32, parentPath string) {
	...
	for i := 0; i < childCount; i++ {
		...
		if child.ChildrenLength() > 0 {
			...
			// File (leaf node).
			t.Files = append(t.Files, FileNode{
	...
	fileEnd := int32(len(t.Files))
	t.Dirs[dirIdx].FileStart = fileStart
	t.Dirs[dirIdx].FileEnd = fileEnd
